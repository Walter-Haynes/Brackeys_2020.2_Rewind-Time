...
namespace CommonGames.Utilities.Extensions
{
    ...
    public static partial class ContainerExtensions
    {
        ...
        /// </summary>
        [PublicAPI]
        public static void CGOrderBy<T>(this IList<T> list, in Func<T, int> func)
        {
        ...
        /// </summary>
        [PublicAPI]
        public static void CGOrderBySpeciated<T>(this IList<T> list, in Func<T, int> func, in Func<T, int> speciate)
        {
            int __listCount = list.Count;

            for (int i = 1; i < __listCount; i++)
            {
                int __index = i;
                T current = list[__index];
                int currentValue = func(current);
                int currentSpeciateValue = speciate(current);

                while (__index > 0)
                {
                    T other = list[__index - 1];
                    
                    if (currentSpeciateValue > speciate(other)) break;
                    
                    if (currentSpeciateValue == speciate(other))
                    {
                        if (currentValue > func(other)) break;
                    }
                    
                    list[__index] = other;
                    __index--;
                    list[__index] = current;
                }
            }
        }

        /// <summary> Add values of other list to target list. </summary>
        [PublicAPI]
        public static void CGAdd<T>(this IList<T> list, in T elementToAdd) => list.Add(elementToAdd);
        
        /// <summary> Add values of other list to target list. </summary>
        [PublicAPI]
        public static void CGAdd<T>(this IList<T> list, in IList<T> listToAdd) => listToAdd.For(list.Add);

        /// <summary>
        /// Add values of other list to target list that return true when overloaded with target function.
        /// </summary>
        [PublicAPI]
        public static void CGAdd<T>(this IList<T> list, in IList<T> other, in Predicate<T> predicate)
        {
            foreach (T __element in other)
            {
                if(predicate(__element))
                {
                    list.Add(__element);
                }
            }
        ...
        /// </summary>
        public static void CGAdd<T>(this IList<T> list, in T[] other, in Predicate<T> predicate)
        {
        ...
        */
        ...
        public static void CGAddAtLast<T>(this IList<T> list, in T value) where T : class
        {
        ...
        /// <summary> Check if target list contains a value that returns true when overloaded with target function. </summary>
        [PublicAPI]
        public static bool CGContains<T>(this IList<T> list, in Predicate<T> predicate)
        {
            foreach (T __element in list) 
            {
                if (predicate(__element)) return true;
            }
            return false;
        }
        ...
        /// <summary> Create a string from all the entries in an array/list. </summary>
        [PublicAPI]
        public static string GenerateString<T>(this IList<T> list)
        {
            System.Text.StringBuilder __stringBuilder = new System.Text.StringBuilder();
            
            foreach(T __item in list)
            {
                __stringBuilder.Append($"{__item.ToString()}, ");
            }

            return __stringBuilder.ToString();
        }

        /// <summary> Execute a function for each item in target list. </summary>
        [PublicAPI]
        public static void For<T>(this IList<T> list, in Action<T> action)
        {
        ...
        /// <summary> Execute a function for each item in target array. </summary>
        [PublicAPI]
        ...
        /// <summary> Execute a function for each item in target list. </summary>
        [PublicAPI]
        public static void For<T>(this IList<T> list, in Action<T, int> action)
        {
        ...
        /// <summary> Execute a function for each item in target list. </summary>
        [PublicAPI]
        public static void SafeFor<T>(this List<T> list, in Action<T, int> action) 
            where T : class
        {
            int __length = list.Count;
            for (int __index = 0; __index < __length; __index++)
            {
                if (list[__index] != null)
                {
                    action(list[__index], __index);
                }
            }
        }
