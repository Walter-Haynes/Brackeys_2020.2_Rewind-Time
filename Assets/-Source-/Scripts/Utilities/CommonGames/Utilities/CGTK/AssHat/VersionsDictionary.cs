...
{
    [Serializable]
    public class VersionsDictionary : SerializableDictionary<string,uint>
    {
    }
    ...
    public class SerializableDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        public TValue this[TKey key] 
        {
            get 
            {
                int __index = FindIndex(key);
                if(__index >= 0)
                {
                    return _Values [__index];
                }
                else
                {
                    throw new KeyNotFoundException(message: key.ToString());   
                }
            }
        ...
        public SerializableDictionary (int capacity)
            : this (capacity: capacity, comparer: null)
        {
        }

        public SerializableDictionary (IEqualityComparer<TKey> comparer)
            : this (capacity: 0, comparer: comparer)
        {
        }
