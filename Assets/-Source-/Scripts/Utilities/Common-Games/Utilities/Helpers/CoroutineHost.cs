...
public class CoroutineHost
{
    ...
    {
        GameObject __coroutineHost = new GameObject("Coroutine Host");
        CoroutineHostInstance __chi = __coroutineHost.AddComponent<CoroutineHostInstance>();
        UnityEngine.Object.DontDestroyOnLoad(__coroutineHost);
        return __chi;
    }
    ...
    private void QueueCoroutine(IEnumerator<object> coroutine)
        ...
        private void Update()
        {
            ...
            {
                if(kvp.Key == null || kvp.Key.MoveNext() != false) continue;
                kvp.Value.SetResult(kvp.Key.Current);
                rememberRemoval.Add(kvp.Key);
            }

            if(rememberRemoval.Count <= 0) return;
