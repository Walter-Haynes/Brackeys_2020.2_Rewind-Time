Shader "RetroLookPro/BottomStretchEffect"
{
	...
	float4 FragDist(VaryingsDefault i) : SV_Target
	{
		...
		uv.y = max(uv.y, twitchHorizonal(amplitude,frequency,uv,_TimeX * 100.0).x * (_NoiseBottomHeight / 20));
	...
	{
		float2 uv = i.texcoord;
		uv.y = max(uv.y, twitchHorizonalRand(amplitude,frequency,uv,_TimeX * 100.0).x * (_NoiseBottomHeight / 20));
		half4 color = tex2D(_MainTex, uv);
		float exp = 1.0;
		return float4(pow(color.xyz, float3(exp, exp, exp)), color.a);
	}
